...
(async () => {
  ...
  try{
    newGame({ role: "random" });
    const p = G.me;
    p.hero = HEROES.find(h => h.id === "sunquan");
    p.hand = [
      { id: 900, key: "wuzhong", name: "无中生有", cat: "trick", suit: "♥", num: 10 },
      { id: 901, key: "shan", name: "闪", cat: "basic", suit: "♦", num: 12 },
      { id: 902, key: "juedou", name: "决斗", cat: "trick", suit: "♠", num: 13 },
      { id: 903, key: "shan", name: "闪", cat: "basic", suit: "♥", num: 8 },
    ];
    p.hp = 4; p.maxHp = 4; p.skillUsed = false;
    const adv = AI.advise(p);
    console.log("ADVISE:", JSON.stringify(adv, null, 1));
  }catch(e){ console.log("ADVISE ERROR:", e.message, e.stack.split("\n").slice(1,4).join("\n")); }
  process.exit(0);
  let pass = 0, fail = 0;
  const wins = {};
